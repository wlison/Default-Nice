package com.wlison.view.nice.utils;
...
import com.wlison.view.nice.App;

/**
 * Author: fancyLuo
 * Date: 2016/9/13 09:59
 */
public class ToastUtils {
     ...
     * @param text 文本
     */
    public static void showLongToastCenter(CharSequence text) {
     ...
     * @param resId 资源Id
     */
    public static void showLongToastCenter(@StringRes int resId) {
    ...
    private static void showToast(@StringRes int resId, int duration) {
        showToast(App.getAppContext().getResources().getText(resId).toString(), duration);
    }
     ...
     * @param duration 显示时长
     */
    private static void showToastCenter(@StringRes int resId, int duration) {
        showToastCenter(App.getAppContext().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void showToast(@StringRes int resId, int duration, Object... args) {
        showToast(String.format(App.getAppContext().getResources().getString(resId), args), duration);
    }
    ...
    private static void showToast(CharSequence text, int duration) {
        ...
        if (sToast == null) {
            sToast = Toast.makeText(App.getAppContext(), text, duration);
        } else {
            sToast.setText(text);
     ...
     * @param duration 显示时长
     */
    private static void showToastCenter(CharSequence text, int duration) {
        if (isJumpWhenMore) cancelToast();
        if (sToast == null) {
            sToast = Toast.makeText(App.getAppContext(), text, duration);
